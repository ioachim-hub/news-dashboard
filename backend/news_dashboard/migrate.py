...
from .db import connect, init_db, insert_article_sql
...
def sqlite_to_postgres(sqlite_path: Path = typer.Argument(..., help="Existing SQLite database path")) -> None:
    if not os.getenv("POSTGRES_HOST") and not os.getenv("DATABASE_URL"):
        raise typer.BadParameter("Set POSTGRES_* env vars or DATABASE_URL before migrating")
    if not sqlite_path.exists():
        raise typer.BadParameter(f"SQLite DB not found: {sqlite_path}")
    ...
    with connect() as conn:
        ...
        for row in articles:
            values: tuple[Any, ...] = tuple(row[column] for column in ARTICLE_COLUMNS)
            conn.execute(
                """
    ...
    typer.echo(f"Migrated {len(sources)} source(s) and {len(articles)} article(s) from {sqlite_path}")
